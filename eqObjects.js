...
const eqObjects = function (object1, object2) {
  ...
  if (firstObb.length !== secObb.length) {
    //compairs length of the arrays to make sure they are equal returns false if not equal
    return false;
  }

  for (let keys of firstObb) {
    //looks through keys of first object
    if (Array.isArray(object1[keys]) && Array.isArray(object2[keys])) {
      //checks to see if both values are NOT arrays
      if (!eqArrays(object1[keys], object2[keys])) {
        //if NOT arrays returns false
        return false;
      }
    } else {
      if (object1[keys] !== object2[keys]) {
        //else if they are arrays checks if value are NOT equal
